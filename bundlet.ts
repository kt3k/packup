...
export const bundlet = async function(flpath: string,  pathPrefix: string, distDir?: string): Promise<{options? : any, plugins?: esbuild.Plugin[]}> {
  if (!distDir) {
    return {};
  }
  ...
  for (const bi of bm) {
    ...
    const data = await bundleByEsbuild(src, { platform: 'browser', format: 'esm' });
  ...
  const ignores = (): esbuild.Plugin => ({
    ...
    setup(build: any) {
      // console.debug(x);
      build.onResolve({ filter: new RegExp(names.join('|').replace(/\./g, '[.]')) }, (args: any) => {
  ...
  return { plugins: [ignores()], options: { platform: 'browser', format: 'esm' } };
};
